...
class ScanNetData(object):
    ...
    def __init__(self, root_path, split='train', scannet200=False, scannetpp=False,
                 save_path=None, label_allowlist_file=None, instance_dir=None):
        self.root_dir = root_path
        self.save_path = root_path if save_path is None else save_path
        self.split = split
        self.split_dir = osp.join(root_path)
        self.scannet200 = scannet200
        self.scannetpp = scannetpp
        if instance_dir is not None:
            self.instance_dir = instance_dir
        else:
            self.instance_dir = 'scannetpp_instance_data' if scannetpp else 'scannet_instance_data'
        if self.scannetpp:
            assert label_allowlist_file is not None and osp.exists(label_allowlist_file), \
                'ScanNet++ requires a label allowlist file (e.g., top100_instance.txt).'
            with open(label_allowlist_file) as f:
                self.classes = [line.strip() for line in f if line.strip()]
            self.cat_ids = np.arange(1, len(self.classes) + 1)
        elif self.scannet200:
            self.classes = [
        ...
        if self.scannetpp:
            split_file = osp.join(self.root_dir, 'splits', f'nvs_sem_{split}.txt')
        else:
    ...
    def get_aligned_box_label(self, idx):
        box_file = osp.join(self.root_dir, self.instance_dir,
    ...
    def get_axis_align_matrix(self, idx):
        matrix_file = osp.join(self.root_dir, self.instance_dir,
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            pts_filename = osp.join(self.root_dir, self.instance_dir,
                                    f'{sample_idx}_vert.npy')
            if not osp.exists(pts_filename):
                print(f'skip {sample_idx}: missing {pts_filename}')
                return None
            info = dict()
            ...
            sp_filename = osp.join(self.root_dir, self.instance_dir,
            ...
            if not self.test_mode:
                pts_instance_mask_path = osp.join(
                    self.root_dir, self.instance_dir,
