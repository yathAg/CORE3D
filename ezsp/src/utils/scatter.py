...
def scatter_pca(x, idx, on_cpu=False, algorithm='eigh'):
    ...
    sizes = idx.bincount()
    # Avoid division by zero when some groups are empty (can happen with
    # missing neighbors). Those groups will be handled downstream.
    sizes_safe = sizes.clamp_min(1)

    # Aggregate the covariances as a N_2x(DxD) with scatter_sum
    # and convert it to a N_2xDxD batch of matrices
    upper_triangle = scatter_sum(upper_triangle, idx, dim=0) / sizes_safe.view(-1, 1)
    # Sanitize any non-finite values early to avoid cuSOLVER failures
    upper_triangle = torch.nan_to_num(upper_triangle)
    # Build a symmetric covariance batch (avoid uninitialized garbage)
    cov = torch.zeros((upper_triangle.shape[0], d, d), device=device, dtype=upper_triangle.dtype)
    ...
    if algorithm == 'eigh' and on_cpu:
        device = cov.device
        cov = cov.cpu()
        # Guard against non-finite values which can crash cuSOLVER
        finite_mask = torch.isfinite(cov).all(dim=(1, 2))
        if not finite_mask.all():
            eigenval = torch.zeros((cov.shape[0], d), dtype=cov.dtype, device=cov.device)
            eigenvec = torch.eye(d, dtype=cov.dtype, device=cov.device).expand(cov.shape[0], d, d).clone()
            if finite_mask.any():
                ev, evec = torch.linalg.eigh(cov[finite_mask], UPLO='U')
                eigenval[finite_mask] = ev
                eigenvec[finite_mask] = evec
        else:
        ...
        if not finite_mask.all():
            ...
            try:
                ...
                # Fallback to CPU if cuSOLVER fails for any reason
                cov_cpu = cov.cpu()
                eigenval, eigenvec = torch.linalg.eigh(cov_cpu, UPLO='U')
                eigenval = eigenval.to(device)
                eigenvec = eigenvec.to(device)
