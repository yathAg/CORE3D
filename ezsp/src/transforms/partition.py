...
try:
    ...
    class OmegaConf:
        ...
        def is_list(_obj):
            return False
try:
    from pycut_pursuit.cp_d0_dist import cp_d0_dist  # type: ignore
except Exception:
    cp_d0_dist = None
try:
    from grid_graph import edge_list_to_forward_star  # type: ignore
except Exception:
    edge_list_to_forward_star = None
...
class CutPursuitPartition(Transform):
    ...
    def _process(self, data):
        if cp_d0_dist is None or edge_list_to_forward_star is None:
            raise ModuleNotFoundError(
                "CutPursuitPartition requires 'pycut_pursuit' and 'grid_graph'. "
                "Install them or use GreedyContourPriorPartition instead."
            )
        # Sanity checks
