...
class PretrainedCNN(Transform):
    ...
    def _process(self, data):
        ...
        with torch.no_grad():
            ...
            # Directly forward the PointStage to avoid pulling full SPT stack
            out = self.first_stage(
                data.x,
                data.norm_index(mode=self.norm_mode),
                pos=data.pos,
                diameter=None,
            ...
            data.x = out[0] if isinstance(out, tuple) else out
